/**
 * WATI MCQ Bot - FULL Webhook Controller (Production-ready)
 *
 * Fixes:
 * ✅ "Select Your Level" coming multiple times
 *    - Ignores non-user events (owner/bot/system)
 *    - Strong dedupe using messageId + (from+text+timestamp fallback)
 *    - Step-level "prompt lock" so LEVEL prompt is sent only once per session step
 *
 * ✅ Uses your Data API (GET)
 * ✅ Uses your MCQ service (generate + save + submitAnswer)
 *
 * Requirements (env):
 *  - WATI_API_TOKEN
 *  - WATI_TENANT_ID
 *  - WATI_BASE_URL
 *  - DATA_API_BASE_URL (default: http://31.97.228.184:5555)
 *  - PYTHON_MCQ_API_URL (your existing mcqService uses this)
 *
 * Notes:
 * - Replace mock redis & User with your real ones (imports provided below).
 */
...
/* ========================================================= */
/* IMPORT YOUR REAL REDIS + USER MODEL (UNCOMMENT IN PROD)    */
/* ========================================================= */
...
/* ========================================================= */
/* IMPORT YOUR EXISTING MCQ SERVICE (FROM YOUR OLD CODE)      */
/* ========================================================= */
...
// Minimal user mock (replace with DB model)
const __users = {};
const User = {
  findOne: async (q) => {
    if (!q?.phoneNumber) return null;
    return __users[q.phoneNumber] || null;
  },
  create: async (data) => {
    const u = { ...data, userId: data.userId || uuidv4(), createdAt: new Date() };
    __users[data.phoneNumber] = u;
    return u;
  },
};
...
/* ========================================================= */
/* DEDUPE + IGNORE NON-USER EVENTS                            */
/* ========================================================= */

/**
 * WATI sometimes triggers webhooks again for:
 * - delivery status
 * - system events
 * - owner messages
 * - duplicated retries
 * This filter reduces noise drastically.
 */
...
/**
 * Prevents resending the same prompt repeatedly for the same step.
 * Example: session.step === "LEVEL" -> send prompt only once per 20 seconds.
 */
...
exports.webhookHandler = async (req, res) => {
  ...
  try {
    ...
    let user = await User.findOne({ phoneNumber: phoneDigits });
    ...
    if (mcqRun) {
      ...
      if (!userAnswer) {
        await sendWatiSessionMessage(from, "❌ Please reply with A/B/C or 1/2/3.");
        return;
      }
    ...
    if (session.step === "NUM") {
      ...
      const mcqIds = pythonMcqs.map(() => uuidv4());
      ...
      const mcqs = pythonMcqs.map((mcq, idx) => ({
        mcqId: mcqIds[idx],
